...
SVG_HEADER = bytearray(('<svg width="%s" height="%s" version="1.1" '
...
class SVGOutputVisitor:
    ...
    def visit_image(self, aa_image, xml_header=True):
        ...
        if xml_header:
            self.file_like.write(SVG_XML_HEADER)
        else:
            self.file_like.write(SVG_HEADER % (
                self._num(self.width),
                self._num(self.height)
            ))

        self.visit_shapes(aa_image.shapes)
        self.file_like.write(b"</svg>\n")
    ...
    def _line(self, x1, y1, x2, y2, thick):
        ...
        self.file_like.write(('%s<line x1="%s" y1="%s" x2="%s" y2="%s" '
                              'stroke="%s" stroke-width="%s" />\n' % (
                                  ...
                                  self._num(y2),
                                  self.foreground,
                                  self.line_width*(1+bool(thick)))).encode('utf-8')
        )
    ...
    def _rectangle(self, x1, y1, x2, y2, style=''):
        ...
        self.file_like.write(('%s<rect x="%s" y="%s" width="%s" height="%s" '
                              'stroke="%s" fill="%s" stroke-width="%s" '
                              'style="%s" />' % (
                                 self.indent,
    ...
    def visit_point(self, point):
        self.file_like.write(('%s<circle cx="%s" cy="%s" r="%s" fill="%s" '
                              'stroke="%s" stroke-width="%s" />' % (
                                 self.indent,
    ...
    def visit_circle(self, circle):
        self.file_like.write(('%s<circle cx="%s" cy="%s" r="%s" stroke="%s" '
                              'stroke-width="%s" fill="%s" />' % (
                                  ...
                                  self.foreground,
                                  self.line_width,
                                  self.fillcolor)).encode('utf-8')
        )

    def visit_label(self, label):
        #  font-weight="bold"   style="stroke:%s"
        self.file_like.write(('%s<text x="%s" y="%s" font-family="%s" '
                              'font-size="%s" fill="%s" >\n %s\n%s</text>' % (
                                 self.indent,
    ...
    def visit_arc(self, arc):
        ...
        self.file_like.write(('%s<path d="M%s,%s C%s,%s %s,%s %s,%s" fill="none"'
                              ' stroke="%s" stroke-width="%s" />' % (
                                  self.indent,
